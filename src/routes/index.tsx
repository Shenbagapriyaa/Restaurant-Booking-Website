...
function Index() {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        const res = await restaurantService.getRecommendations();
        if (res.data.success && res.data.dishes.length > 0) {
          const mapped = res.data.dishes.map((d: any) => ({
            name: d.foodName,
            chef: d.chefSpecial ? "Chef's Special Selection" : "Maison Partner Chef",
            image: d.foodImage,
            price: `$${d.price}`,
          }));
        ...
        console.log("Using static fallback for recommendations");
      }
    };

    const fetchFeatured = async () => {
      try {
        const res = await restaurantService.getAll({ featured: "true" } as any);
        ...
        console.log("Using static fallback for featured restaurants");
      }
    };
